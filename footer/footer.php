<!--====== Side Button Call BAck Start ======-->
<div class="call-back-btn">
    <a href="#" data-toggle="modal" data-target="#exampleModalnew">Book Now</a>
</div>

<div class="social-icons-sidebar">
    <ul class="social-sidebar">
        <li class="fb-one">
            <a href="" class="facebook" target="_blank">
                <i class="fab fa-facebook-f"></i>
                <span>Facebook</span>
            </a>
        </li>
        <li class="fb-two">
            <a href="" class="facebook" target="_blank">
                <i class="fab fa-twitter"></i>
                <span>Twitter</span>
            </a>
        </li>
        <li class="fb-three">
            <a href="" class="facebook" target="_blank">
                <i class="fab fa-instagram"></i>
                <span>Instagram</span>
            </a>
        </li>
        <li>
            <a id="" href="tel:+91481 2303722" class="facebook" target="">
                <i class="fas fa-phone-alt"></i>
                <span>Call&nbsp;Now</span>
            </a>
        </li>
        <li>    
            <a id="" href="https://wa.me/+914812303722" class="facebook" target="_blank">
                <i class="fab fa-whatsapp"></i>
                <span>Whatsapp&nbsp;Now</span>
            </a>
        </li>
    </ul>
</div>

<div class="button-icons-sidebar">
    <ul class="button-sidebar">
        <li class="fb-one">
            <a href="covid-test.php" class="sidebar_btn">
                <div><img src="assets/img/popup/corona.png" alt=""></div>
               <div><h5>Covid Test Booking</h5></div> 
                <span>Covid Test Booking</span>
            </a>
        </li>
        <li class="fb-two">
            <a href="home-collection.php" class="sidebar_btn">
            <div><img src="assets/img/popup/health-package.png" alt=""></div>
            <div><h5>Book Home Collection</h5></div> 
                <span>Book Home Collection</span>
            </a>
        </li>
        <li class="fb-three">
            <a href="health-package.php" class="sidebar_btn">
            <div><img src="assets/img/popup/home-collection.png" alt=""></div>
            <div><h5>Health Packages Details</h5></div> 
                <span>Health Packages Details</span>
            </a>
        </li>
    </ul>
</div>


<!--====== Footer Area Start ======-->
<footer class="section">
    <div class="footer-bg-bottom"></div>
    <div class="container">
        <div class="row border-bottom">
            <div class="col-lg-9">
                <div class="footer-top ft-1">
                    <h3>Services</h3>
                    <div class="footer-menu-area d-flex">
                        <ul>
                            <!-- <li><a href="biochemistry.php">Biochemistry</a></li> -->
                            <li><a href="biochemistry.php">Medical Biochemistry</a></li>
                            <li><a href="microbiology.php">Medical Microbiology</a></li>
                            <li><a href="ct-scan.php"> CT Scan</a></li>
                            <li><a href="mammogram.php"> Mammogram</a></li>
                            <li><a href="ultrasound-scanning.php"> Ultrasound Scanning</a></li>
                            <li><a href="colour-doppler.php"> Colour Doppler</a></li>
                           
                            <!-- <li><a href="doppler-ultrasound.php">Doppler Ultrasound</a></li> -->
                            <!-- <li><a href="serology.php">Serology</a></li>
                            <li><a href="radiology.php">Radiology</a></li> -->
                          
                        </ul>
                        <ul>
                            <!-- <li><a href="microbiology.php">Microbiology</a></li> -->
                            <li><a href="endocrinology.php">Endocrinology</a></li>
                            <li><a href="allergy-profiles.php">Allergy Profiles</a></li>
                            <!-- <li><a href="imaging-ultrasound-scanning.php"></a></li> -->
                            <li><a href="histopathology.php">Histopathology</a></li>
                            <li><a href="opg.php">OPG </a></li>
                            <li><a href="x-ray.php">X-RAY</a></li>
                            <li><a href="cytopathology.php">Cytopathology</a></li>
                        </ul>
                        <ul class="third-ul">
                            <li><a href="clinical-pathology.php">Clinical Pathology</a></li>
                            <li><a href="hematology.php">Hematology</a></li>
                            <!-- <li><a href="clinical-chemistry.php">Clinical Chemistry</a></li> -->
                            <li><a href="immunology.php">Immunology and serology</a></li>
                            <li><a href="molecular-biology.php">Molecular Biology</a></li>
                            <li><a href="prenatal-screening.php">Prenatal Screening</a></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="footer-top ft-2">
                    <h3>Links</h3>
                    <div class="footer-menu-area">
                        <ul>
                            <li><a href="about.php">About Us</a></li>
                            <li><a href="health-package.php">Packages</a></li>
                            <li><a href="spacialities.php">Specialities</a></li>
                            <li><a href="careers.php">Careers</a></li>
                            <li><a href="branches.php">Branches</a></li>
                            <li><a href="contact.php">Contact Us</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <div class="row border-bottom pt-3 pb-3 p-25">
            <div class="col-lg-3">
                <div class="ft-logo">
                    <img src="assets/img/logo/dianova-white-logo.png" alt="">
                </div>
            </div>
            <div class="col-lg-3">
                <div class="ft-address">
                    <p>Opp. Baselious College,
                        Manorama Junction,
                        Good Shepherd Road, Kottayam-1</p>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="ft-contact d-flex align-items-center">
                    <div class="icon">
                        <i class="fas fa-phone-alt"></i>
                    </div>
                    <div class="num">
                        <a href="tel:04812303722">0481 2303722</a>
                        <a href="tel:04812583722" class="num-last">0481 2583722</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="ft-social">
                    <div class="mail">
                        <a href="mailto:dianovalaborateries@gmail.com"><i class="far fa-envelope"></i> dianovalaborateries@gmail.com</a>
                    </div>
                    <div class="icons">
                        <ul>
                            <li>
                                <a href="" class="text-white-50"><i class="fab fa-facebook-f"></i></a>
                            </li>
                            <li>
                                <a href="" class="text-white-50"><i class="fab fa-twitter"></i></a>
                            </li>
                            <li>
                                <a href="" class="text-white-50"><i class="fab fa-instagram"></i></a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <div class="copyright-area d-flex flex-wrap justify-content-center justify-content-sm-between text-center py-4">
                    <div class="copyright-left text-white-50">Copyright &copy; 2021 Dianova | All Rights Reserved. Designed by <a href="http://www.theviralmafia.com/?utm_source=dianova" target="_blank"> <img src="assets/img/logo/viralmafia.svg"></a></div>
                </div>
            </div>
        </div>
    </div>
</footer>